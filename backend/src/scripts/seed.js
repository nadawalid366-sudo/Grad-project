...
const DB_NAME = process.env.DB_NAME || "vitalConnect";
...
async function seed() {
  ...
  try {
    ...
    await db.collection("users").deleteMany({ email: { $in: [patient1, patient2, doctor1] } });
    ...
    await db.collection("users").insertMany([
      ...
      },
      {
        ...
        password: hashedPassword,
        role: "professional",
        profile: { fullName: "Dr. Gregory House", specialty: "Diagnostic Medicine" },
        createdAt: new Date()
      }
    ]);
