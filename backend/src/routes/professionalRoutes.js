...
const CONSULTATION_TYPES = ["free", "paid"];
...
function publicProfessional(doc) {
  if (!doc) return null;
  const type = PROFESSIONAL_TYPES.includes(doc.type) ? doc.type : "doctor";
  const consultationType = CONSULTATION_TYPES.includes(doc.consultationType)
    ? doc.consultationType
    : "paid";
  const price = consultationType === "free"
    ? 0
  ...
  try {
    ...
    if (
      ...
      return res.status(400).json({
        message:
          "Full name, email, password, specialty, experience, bio, and consultation type are required.",
      });
    }

    const normalizedType = PROFESSIONAL_TYPES.includes(type) ? type : "doctor";
    const normalizedConsultationType = CONSULTATION_TYPES.includes(consultationType)
      ? consultationType
      : null;

    if (!normalizedConsultationType) {
      return res.status(400).json({
        message: "Consultation type must be either 'free' or 'paid'.",
      });
    }

    let normalizedPrice = 0;
    if (normalizedConsultationType === "paid") {
      const parsedPrice = Number(price);
      if (!Number.isFinite(parsedPrice) || parsedPrice <= 0) {
        return res.status(400).json({
          message: "Monthly subscription price is required for paid consultation.",
        });
      }
...
// Assign a patient to the authenticated doctor (create patient-doctor link)
router.post(
  "/assign-patient",
  requireAuth,
  ...
  async (req, res) => {
    try {
      ...
      const normalizedDoctorEmail = req.auth.email;

      if (!patientEmail) {
        return res
          .status(400)
          .json({ message: "Patient email is required." });
      }
