...
export default function AlertsScreen() {
  ...
  useEffect(() => {
    ...
    fetchDoctorAlerts(doctorEmail)
      .then((response) => {
        if (!active) return;
        setDashboardAlerts(
          (response.alerts || []).map((alert: any, index: number) => ({
  ...
  const renderAlertCard = (alert: Alert) => {
    ...
    return (
      <View
        key={alert.id}
        style={[styles.alertCard, { borderLeftColor: colors.border }]}
      >
        ...
        <View style={styles.alertActions}>
          ...
          <TouchableOpacity
            ...
            onPress={() =>
              resolveDoctorAlert(doctorEmail, alert.id).catch((error) =>
                console.log("Failed to resolve alert:", error),
              )
            }
          >
          ...
          {filteredAlerts.length > 0 ? (
            ...
            <View style={styles.emptyState}>
              ...
              <Text style={styles.emptyStateText}>
                There are no {selectedSeverity.toLowerCase()} alerts at the
                moment
              </Text>
            </View>
          )}
